...
class GuisModels:
    ...
    def harmonic(
        inputFile, window, M, N, t, minSineDur, nH, minf0, maxf0, f0et, harmDevSlope
    ):
    ...
    def spr(
        inputFile, window, M, N, t, minSineDur, maxnSines, freqDevOffset, freqDevSlope
    ):
        """
        inputFile: input sound file (monophonic with sampling rate of 44100)
    ...
    def stochastic(inputFile, H, N, stocf=0.1):
        """
        inputFile: input sound file (monophonic with sampling rate of 44100)
    ...
    def plot_harmonic_sinusoidal_freqs(
        x, fs, freqs, H, max_plot_freq=4000, freq_type="harmonic"
    ):
        """
    ...
    def plot_harmonic_on_top(
        hfreq, fs, H, color="k", alpha=1, max_plot_freq=4000, freq_type="Stochastic"
    ):
        if hfreq.shape[1] > 0:
...
class GuiController:
    ...
    def update_json(self, data, key):
        with open(
            f"{GuiView.DIR_NAME}/results.json".format(GuiView.DIR_NAME), "r+"
        ) as f:
    ...
    def compute_model_results(
        self, json, result_dict, model_dict, filename, call_model
    ):
...
class GuiView:
    ...
    def __init__(self, model, controller):
        ...
        self.hop_size = widget.IntText(
            value=512, description="Hop Size (H)", style=self.style
        )

        self.magnitude_threshold = widget.IntText(
            value=-80, description="Magnitude Threshold (t)", style=self.style
        )
        ...
        self.select_audio = widgets.FileUpload(
            description="Select Audio File", accept=".wav", multiple=False
        )

        compute = widget.Button(
            description="Compute",
            button_style="warning",
            layout=widgets.Layout(width="50%", height="40px"),
        )
        mag_spectrum = widget.Button(
            description="Magnitude Spectrum",
            button_style="warning",
            layout=widgets.Layout(width="50%", height="40px"),
        )
        mag_spectogram = widget.Button(
            description="Magnitude Spectogram",
            button_style="warning",
            layout=widgets.Layout(width="50%", height="40px"),
        )
        phase_spectrum = widget.Button(
        ...
        input_sound = widget.Button(
            description="Input Sound",
            button_style="success",
            layout=Layout(width="50%", height="40px"),
        )
        output_sound = widget.Button(
        ...
        def plot(_):
            ...
            self.model_name = self.model_names[self.tab.selected_index]

            # Get the model function
            model_widget = getattr(self, self.model_name)
            self.model_func = getattr(model_widget, self.model_name)

            self.input_filename = self.controller.get_selected_audio_filename(
                self.select_audio
            )
            results = self.controller.compute_results(
                self.input_filename, self.model_func, model_widget
            )
            self.model.plot_magnitude_phase_spectrum(
                results["fs"], results["mX"], results["Window Size (M)"]
            )

        @mag_spectogram.on_click
        def plot_mag(_):
            results = self.controler.compute_results()
            self.model.plot_magnitude_spectogram(
                results["fs"], results["N"], results["mX"]
            )
